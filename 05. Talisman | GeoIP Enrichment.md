...
Install geoipupdate, configure /etc/GeoIP.conf with AccountID/LicenseKey/EditionIDs, run geoipupdate, add cron: 0 3 * * 0 /usr/bin/geoipupdate.  
...
**Update es_suricata inputs in sinks with ["pfsense_geo_point"]**
```
es_pfsense:
    type: elasticsearch
...
**Suricata Enrichment**
- Add in transforms after suricata_parse:
```
suricata_geoip_src:
  type: remap
  inputs: ["suricata_parse"]
  source: |
    geo, err = get_enrichment_table_record("geoip_city", { "ip": .suricata.src_ip })
    if err == null { .source.geo = geo } else { .source.geo = {} }

suricata_geoip_dest:
  type: remap
  inputs: ["suricata_geoip_src"]
  source: |
    geo, err = get_enrichment_table_record("geoip_city", { "ip": .suricata.dest_ip })
    if err == null { .destination.geo = geo } else { .destination.geo = {} }

suricata_geo_point:
  type: remap
  inputs: ["suricata_geoip_dest"]
  source: |
    if exists(.source.geo.latitude) && exists(.source.geo.longitude) {
      lat = to_float!(.source.geo.latitude)
      lon = to_float!(.source.geo.longitude)
...
**Update es_suricata inputs in sinks with ["suricata_geo_point"]**
```
es_suricata:
    type: elasticsearch
    inputs: ["suricata_geo_point"]
```

**Zeek Enrichment**
- Add in transforms after zeek_parse:
```
zeek_geoip_src:
  type: remap
  inputs: ["zeek_parse"]
...
**Update es_zeek inputs in sinks with ["zeek_geo_point"]**
```
es_zeek:
    type: elasticsearch
    inputs: ["zeek_geo_point"]
```
...
- Replace INDEX with suricata-ecs, pfsense-ecs, zeek-ecs, wazuh-alerts-ecs for each request:
...
## Final Steps
- Delete old indices
**Find indices by GET request at Dev tools - GET /_cat/indices**
**Delete indice with POST request - DELETE /INDEX-YYYY.MM.DD - examople: DELETE zeek-ecs-1881.07.21**
- Restart Vector
