...
input {
  file {
    ...
    mode => "tail"
    start_position => "end"
    sincedb_path => "/var/lib/logstash/sincedb/suricata.sincedb"
    stat_interval => 1
...
filter {
  if "source_suricata" in [tags] {
    ...
    # If you don’t want stats indexed:
...
2. Select your **"Suricata Events"** data view
3. You should see Suricata alerts appearing in real-time.
