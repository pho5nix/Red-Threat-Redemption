...
This guide covers preparing a Proxmox VM, performing a minimal Debian 13 installation, and post-install configuration optimized for the SIEM stack (32GB RAM, 8 cores, PCI passthrough NIC for Zeek).
...
- Disable root login and password auth:  
  ...
  ```  
- Use SSH keys for auth (generate with `ssh-keygen`; copy public key to `~/.ssh/authorized_keys`).
